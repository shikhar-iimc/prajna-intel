...
def generate_pdf_brief(brief_week, top_pairs, headlines, brief_text, names):
    ...
    pdf.cell(0, 6, str(len(names)) + " entities tracked  |  " + str(len(top_pairs)) + " signal pairs  |  AUTO-GENERATED", ln=True, align="C")
    pdf.ln(6)
    ...
    for line in brief_text_safe.split("\n"):
        ...
        if not line:
            pdf.ln(3)
        elif any(line.startswith(x) for x in ["STORY", "BOTTOM LINE", "WEEK", "EXECUTIVE", "STRATEGIC RISK", "HIGH RISK", "MEDIUM RISK", "OPPORTUNITY"]):
    ...
    pdf.multi_cell(0, 5, "This brief was auto-generated by Prajna using knowledge graph analysis of real-time news data. For strategic assessment purposes only.")
...
def build_graph_visual(keyword=None, week=None, mode="semantic"):
    ...
    with driver.session() as session:
        if mode == "semantic":
            # ── Top nodes by semantic relationship count ──
            if keyword:
                res = session.run("""
                ...
                res = session.run("""
                    ...
                    WITH e, COUNT{(e)-[:RELATES_TO]-()} AS conn
                    ORDER BY conn DESC LIMIT 40
            ...
            if week:
                ...
                for r in rels:
                    ...
                    w = wc.get(week, 0)
                    if w >= 1:
                        relationships.append({"source": r["source"], "target": r["target"], "weight": w, "rel_type": "CO_OCCURS_WITH"})
            else:
                rels = session.run("""
                    MATCH (e1:Entity)-[r:CO_OCCURS_WITH]-(e2:Entity)
                ...
                """, {"names": list(top.keys())})
                relationships = [{"source": r["source"], "target": r["target"], "weight": r["weight"], "rel_type": "CO_OCCURS_WITH"} for r in rels]
    ...
    for r in relationships:
        if r["source"] in top and r["target"] in top:
            ...
            label      = rel_type.replace("_", " ") if mode == "semantic" else ""
            net.add_edge(
                r["source"], r["target"],
                value=min(r["weight"] * 0.4, 4),
                title=rel_type + ": " + str(r["weight"]) + "x",
                label=label,
                color={"color": edge_color, "highlight": "#E8D5A3"},
                font={"size": 7, "color": edge_color, "face": "IBM Plex Mono"}
            )
...
.rel-card { border: 1px solid #1C2A38; background: #0D1117; padding: 10px 14px; margin: 4px 0; display: flex; align-items: center; gap: 12px; font-family: 'IBM Plex Mono', monospace; font-size: 11px; }
.rel-type { font-size: 9px; padding: 2px 8px; border-radius: 0; font-weight: 600; letter-spacing: 0.1em; }
...
with tab1:
    ...
    with col_left:
        ...
        query = st.text_input("STRATEGIC QUESTION", placeholder="What are India's energy security risks from Iran?", value=st.session_state["query_input"])
        ...
        if run_query and query:
            with st.spinner(""):
                ...
                prompt = (
                    ...
                    "KEY CONNECTIONS — 3-4 bullet points from graph data, cite relationship types\n"
    ...
    with col_right:
        ...
        with c_filter:
            graph_filter = st.text_input("FILTER BY ENTITY", placeholder="Iran, China, Taliban...", key="graph_filter")
        with c_mode:
            graph_mode = st.selectbox("VIEW MODE", ["SEMANTIC", "CO-OCCURRENCE"], key="graph_mode")
        with c_week:
            selected_week = st.selectbox("WEEK", week_options, key="graph_week")
            ...
            try:
                ...
                if mode_param == "semantic":
                    ...
                    for color, label in legend_items:
                        legend_html += f'<div style="display:flex;align-items:center;gap:4px;font-family:IBM Plex Mono;font-size:8px;color:{color}"><div style="width:8px;height:8px;background:{color};border-radius:50%"></div>{label}</div>'
                    legend_html += '</div>'
                    st.markdown(legend_html, unsafe_allow_html=True)
                else:
                    week_label = f"Showing: {selected_week}" if week_param else "Showing: All time"
                    st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:9px;color:#2A3A4A;text-transform:uppercase;letter-spacing:0.1em;margin-top:8px">{week_label} &nbsp;·&nbsp; Node size = connection strength</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if run_traj:
        ...
        if not weekly_data:
            ...
            # Also show semantic relationships between these two entities
            with driver.session() as session:
            ...
            prompt = (
                ...
                "Total: " + str(total) + "\n\n"
                "Semantic relationship types extracted from articles:\n" + sem_str + "\n\n"
                "SITUATION — is this relationship intensifying, cooling, or volatile?\n"
                "RELATIONSHIP TYPE — what kind of relationship is this based on semantic data?\n"
...
with tab3:
    ...
    with subtab2:
        st.markdown('<div style="font-family:IBM Plex Mono;font-size:10px;color:#3A5068;margin-bottom:20px;line-height:1.7">Full semantic relationship profile for any entity — all typed relationships extracted from the knowledge graph.</div>', unsafe_allow_html=True)
        ...
        if run_sem:
            ...
            if not outgoing and not incoming:
                ...
                prompt = (
                    ...
                    "OUTGOING RELATIONSHIPS (what it does to others):\n" + sem_out_str + "\n\n"
                    "INCOMING RELATIONSHIPS (what others do to it):\n" + sem_in_str + "\n\n"
