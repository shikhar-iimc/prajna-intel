...
with tab5:
    ...
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:10px;color:#3A5068;margin-bottom:20px;line-height:1.7">Prajna automatically identifies the 5 most strategically significant developments<br>India should monitor — zero user input required.</div>', unsafe_allow_html=True)
    ...
    if run_weekly_brief:
        ...
        with st.spinner("Analysing graph..."):
            with driver.session() as session:
                ...
                pairs = session.run(
                    "MATCH (a:Article {week:$week})-[:MENTIONS]->(e1:Entity) "
                ...
                for p in top_pairs:
                    arts = session.run(
                        "MATCH (a:Article {week:$week})-[:MENTIONS]->(e1:Entity {name:$e1}) "
                        "MATCH (a)-[:MENTIONS]->(e2:Entity {name:$e2}) "
                        "RETURN a.title AS title, a.source AS source LIMIT 3",
                        {"week": brief_week, "e1": p["e1"], "e2": p["e2"]}
                    ).data()
                    headlines[f"{p['e1']}↔{p['e2']}"] = arts

            pairs_str = "
".join([f"  {p['e1']} ↔ {p['e2']}: {p['co_count']} co-mentions" for p in top_pairs])
            headlines_str = ""
            for pair_key, arts in headlines.items():
                headlines_str += f"
{pair_key}:
"
                for a in arts:
                    headlines_str += f"  [{a['source']}] {a['title']}
"

            prompt = (
                f"You are Prajna, India's strategic intelligence engine.
"
                f"Week: {brief_week}

"
                f"Most active entity relationships this week (knowledge graph):
{pairs_str}
...
"
                f"Generate a WEEKLY INTELLIGENCE BRIEF for Indian policymakers.
"
                f"Format exactly as:

"
                f"WEEK {brief_week} — STRATEGIC INTELLIGENCE BRIEF
...
"
                f"BOTTOM LINE FOR INDIA: One sentence strategic summary.

"
                f"Be direct, analytical, India-centric. Max 400 words."
            )
        ...
        source_tags = "".join([f'<span class="source-tag">{s}</span>' for s in all_sources[:8]])

        brief_html = (
            '<div class="brief-container">'
            '<div class="brief-header">-- WEEKLY BRIEF &nbsp;·&nbsp; '
            + brief_week + ' &nbsp;·&nbsp; '
            + str(len(top_pairs)) + ' signal pairs &nbsp;·&nbsp; '
            + str(len(names)) + ' entities tracked &nbsp;·&nbsp; ZERO USER INPUT'
            '</div>'
            + brief_text.replace("
", "<br>")
            + '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:12px;'
            'padding-top:12px;border-top:1px solid #1C2A38">'
            + source_tags
            + '</div></div>'
        )
