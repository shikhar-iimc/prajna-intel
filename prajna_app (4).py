...
def generate_pdf_brief(brief_week, top_pairs, headlines, brief_text, names):
    class PDF(FPDF):
        ...
        def footer(self):
            ...
            self.cell(0, 10, "PRAJNA  |  " + brief_week + "  |  CONFIDENTIAL — FOR AUTHORISED USE ONLY  |  Page " + str(self.page_no()), align="C")
    ...
    pdf.cell(0, 8, "WEEKLY INTELLIGENCE BRIEF  —  " + brief_week, ln=True, align="C")
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(74, 106, 138)
    pdf.cell(0, 6, str(len(names)) + " entities tracked  |  " + str(len(top_pairs)) + " signal pairs  |  AUTO-GENERATED  |  ZERO USER INPUT", ln=True, align="C")
    pdf.ln(6)
    ...
    for p in top_pairs:
        ...
        pdf.cell(0, 5, "  " + p["e1"] + "  <>  " + p["e2"], ln=True)
        arts = headlines.get(p["e1"] + " and " + p["e2"], [])
        for a in arts:
            pdf.set_font("Helvetica", "", 8)
            pdf.set_text_color(90, 122, 154)
            pdf.multi_cell(0, 5, "    [" + a["source"] + "]  " + a["title"])
        pdf.ln(1)
    ...
    pdf.multi_cell(0, 5, "This brief was auto-generated by Prajna using knowledge graph analysis of real-time news data. It is intended for strategic assessment purposes only. All relationships are derived from article co-occurrence patterns in the Prajna Neo4j graph database.")
    ...
    with driver.session() as session:
        ...
        if week:
            ...
            for r in rels:
                ...
                w = wc.get(week, 0)
                if w >= 1:
                    relationships.append({"source": r["source"], "target": r["target"], "weight": w})
        else:
            rels = session.run("""
                MATCH (e1:Entity)-[r:CO_OCCURS_WITH]-(e2:Entity)
...
with tab1:
        ...
        if run_query and query:
            with st.spinner(""):
                ...
                prompt = (
                    ...
                    "Answer ONLY from the graph context below.\n\n"
                    "KNOWLEDGE GRAPH CONNECTIONS:\n" + ctx_str + "\n\n"
                    "NEWS SOURCES:\n" + art_str + "\n\n"
                    "QUESTION: " + query + "\n\n"
                    "Structure your response as:\n"
                    "SITUATION — 2-3 sentence summary\n"
                    "KEY CONNECTIONS — 3-4 bullet points from graph data\n"
            ...
            st.markdown(
                ...
                + ' &nbsp;·&nbsp; ' + str(len(context)) + ' graph connections analysed</div>'
                + brief.replace("\n", "<br>") + sources_html + '</div>',
                unsafe_allow_html=True
            )
    ...
    with col_right:
        ...
        with c_week:
            selected_week = st.selectbox("WEEK VIEW", week_options, key="graph_week")
        week_param = None if selected_week == "ALL TIME" else selected_week
        with st.spinner(""):
            try:
                graph_html = build_graph_visual(keyword=graph_filter or None, week=week_param)
                components.html(graph_html, height=460)
                week_label = f"Showing: {selected_week}" if week_param else "Showing: All time"
                st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:9px;color:#2A3A4A;text-transform:uppercase;letter-spacing:0.1em;margin-top:8px">{week_label} &nbsp;·&nbsp; Node size = connection strength &nbsp;·&nbsp; Drag to explore</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if run_traj:
        ...
        if not weekly_data:
            ...
            prompt = (
                "You are Prajna. Analyse this relationship trajectory:\n\n"
                + e1 + " vs " + e2 + "\n"
...
with tab3:
    ...
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:10px;color:#3A5068;margin-bottom:20px;line-height:1.7">Finds the shortest connection path between any two entities in the knowledge graph.<br>Surfaces non-obvious relationships invisible to human analysts.</div>', unsafe_allow_html=True)
    c1, c2, c3 = st.columns([2, 2, 1])
