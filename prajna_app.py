...
def get_trajectory(e1, e2):
    with driver.session() as session:
        ...
        """, {"e1": e1, "e2": e2}).single()
    if not result or not result["wcj"]: return None, None
...
.prajna-nav { display:flex; align-items:center; justify-content:space-between; padding:0 0 20px 0; border-bottom:1px solid #1C2A38; margin-bottom:28px; }
...
tab1, tab2, tab3, tab4 = st.tabs([
    "INTELLIGENCE BRIEF", "TRAJECTORY ANALYSIS",
    "PATH QUERY", "SURGE DETECTION"
])
...
Answer ONLY from the graph context below. Cite every claim.
...
SOURCES — list cited sources"""
            ...
            <div class="brief-container">
                <div class="brief-header">▪ INTELLIGENCE BRIEF &nbsp;·&nbsp;
                {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")}
                &nbsp;·&nbsp; {len(context)} graph connections analysed</div>
    ...
    with col_right:
        ...
        graph_filter = st.text_input("FILTER BY ENTITY",
            placeholder="Iran, China, Taliban...", key="graph_filter")
        with st.spinner(""):
            try:
                graph_html = build_graph_visual(graph_filter or None)
                components.html(graph_html, height=460)
                st.markdown('<div style="font-family:IBM Plex Mono;font-size:9px;color:#2A3A4A;text-transform:uppercase;letter-spacing:0.1em;margin-top:8px">Node size = connection strength &nbsp;·&nbsp; Drag to explore &nbsp;·&nbsp; Hover for details</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if run_traj:
        ...
        if not weekly_data:
            st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#5A3A3A;border:1px solid #2A1A1A;padding:14px 18px;background:#0D0A0A">▪ NO DATA — {e1.upper()} ↔ {e2.upper()}<br><span style="color:#2A3A4A;font-size:9px">Try: India/Iran · Iran/Israel · India/Russia</span></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:10px;color:#4A6A4A;border:1px solid #1A2A1A;padding:10px 16px;background:#0A0D0A;margin-bottom:16px">▪ {e1.upper()} ↔ {e2.upper()} &nbsp;·&nbsp; {total} TOTAL CO-OCCURRENCES &nbsp;·&nbsp; {len(weekly_data)} WEEKS</div>', unsafe_allow_html=True)
            ...
            prompt = f"""Analyse trajectory for {e1} <-> {e2}:
{traj_str}
Total all-time: {total}

SITUATION — intensifying, cooling, or volatile?
INFLECTION POINTS — sharp changes and likely causes
INDIA STRATEGIC IMPLICATIONS — what does this mean for India
WATCH FOR — specific developments to monitor
Max 250 words."""
            with st.spinner(""):
                brief = ask_groq(prompt)
            st.markdown(f'<div class="brief-container"><div class="brief-header">▪ TRAJECTORY — {e1.upper()} ↔ {e2.upper()}</div>{brief.replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)

# ── TAB 3: PATH QUERY ──
with tab3:
    st.markdown('<div class="section-label">Graph Path Query</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:10px;color:#3A5068;margin-bottom:20px;line-height:1.7">Finds shortest connection path between any two entities.<br>Surfaces non-obvious relationships invisible to human analysts — impossible for LLMs.</div>', unsafe_allow_html=True)
    ...
    if run_path:
        ...
        if not path:
            st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#5A3A3A;border:1px solid #2A1A1A;padding:14px 18px;background:#0D0A0A">▪ NO PATH — {p1.upper()} → {p2.upper()}<br><span style="color:#2A3A4A;font-size:9px">No connection within 4 hops. Try different entities.</span></div>', unsafe_allow_html=True)
        else:
            nodes = path["nodes"]; strengths = path["strengths"]
            ...
            for i, node in enumerate(nodes):
                cls = "start" if i==0 else ("end" if i==len(nodes)-1 else "mid")
                sym = "▶" if i==0 else ("◼" if i==len(nodes)-1 else "◦")
                nodes_html += f'<div class="path-node {cls}">{sym} &nbsp; {node.upper()}</div>'
                path_str   += node
                if i < len(strengths):
            ...
            prompt = f"""Graph path found: {path_str} ({path["hops"]} hops)
Each arrow = entities co-appearing in real news. Number = frequency.

HIDDEN CONNECTION — what does this mean in plain English?
...
with tab4:
    ...
    st.markdown('<div style="font-family:IBM Plex Mono;font-size:10px;color:#3A5068;margin-bottom:20px;line-height:1.7">Monitors graph for entities whose news co-occurrence is growing anomalously fast.<br>No query required — Prajna surfaces what you did not know to ask about.</div>', unsafe_allow_html=True)
    ...
    if run_surge:
        ...
        if not surges:
            st.markdown(f'<div style="font-family:IBM Plex Mono;font-size:11px;color:#3A5068;border:1px solid #1C2A38;padding:14px 18px;background:#0D1117">▪ NO SURGES ABOVE {threshold}× THRESHOLD<br><span style="font-size:9px;color:#2A3A4A">Surge detection strengthens as more weeks accumulate in graph.</span></div>', unsafe_allow_html=True)
            with driver.session() as session:
                top = session.run("""
                    MATCH (e:Entity)-[r:CO_OCCURS_WITH]-()
                ...
                """).data()
            rows = "".join([f'<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 12px;border-bottom:1px solid #0D1117;font-family:IBM Plex Mono;font-size:10px;color:#5A7A9A"><span>{r["name"].upper()}</span><span style="color:#3A5068">{r["conn"]} connections</span></div>' for r in top if len(r["name"])<35])
            ...
            for surge in surges:
                ...
                prompt = f"""Surge alert: {surge["entity"]} ({surge["type"]})
This week: {surge["latest"]} connections. Baseline: {surge["baseline"]}. Ratio: {surge["ratio"]}×
SURGE EXPLANATION — why is this surging?
