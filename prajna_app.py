...
def sanitize(text):
    ...
    # break up any word longer than 60 chars (URLs, etc) to prevent fpdf crash
    words = text.split(" ")
    broken = []
    for w in words:
        if len(w) > 60:
        ...
        if mode == "semantic":
            if keyword:
                res = session.run("""
                    ...
                    UNWIND seed AS s
                    MATCH (s)-[:RELATES_TO]-(nb:Entity)
                    WITH COLLECT(DISTINCT s) + COLLECT(DISTINCT nb) AS nl
                    UNWIND nl AS node
                    WITH DISTINCT node
                    MATCH (node)-[r:RELATES_TO]-()
        ...
        with st.spinner(""):
            try:
                ...
                if mode_param == "semantic":
                    ...
                    st.markdown('<div style="font-family:IBM Plex Mono;font-size:9px;color:#2A3A4A;text-transform:uppercase;letter-spacing:0.1em;margin-top:8px">' + week_label + ' &nbsp;·&nbsp; Node size = connection strength</div>', unsafe_allow_html=True)
