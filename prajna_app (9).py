...
def pdf_write_line(pdf, line):
    ...
    # Handles header lines vs body lines differently
    line = line.strip()
    if not line:
        pdf.ln(3)
        return
...
# Called from Tab 5 Weekly Brief after detailed_brief is generated
...
def get_graph_context(keywords):
    # Used by Tab 1 intelligence brief — queries RELATES_TO for semantic context
    with driver.session() as session:
        result = session.run("""
...
def get_articles(keywords):
    # Used by Tab 1 — fetches supporting article titles and sources
    with driver.session() as session:
        result = session.run("""
...
def get_trajectory(e1, e2):
    # Used by Tab 2 — weekly co-occurrence counts between two entities
    with driver.session() as session:
        rows = session.run("""
...
def get_semantic_relations(entity):
    # Used by Tab 3 Entity Semantic Profile
    # Returns outgoing (entity acts on others) and incoming (others act on entity)
    with driver.session() as session:
...
def find_path(e1, e2):
    # Used by Tab 3 Path Query — shortest path via CO_OCCURS_WITH
    with driver.session() as session:
        result = session.run("""
...
# Blocklist — entities to exclude from surge detection (media orgs, generic terms)
BLOCKLIST = {
...
def detect_surges(threshold=1.5, top_n=5):
    # Compares current week article mentions vs historical baseline
    # Returns entities surging above threshold x baseline
...
# Maps RELATES_TO relationship types to display colors
# Red = conflict, Orange = military, Yellow = sanctions/oppose
# Blue = trade/economic, Green = alliance, Purple = governance, Teal = diplomacy
RELATION_COLORS = {
    "THREATENS":              "#C87B6A",
    "ATTACKS":                "#C87B6A",
...
# mode = "semantic"     → uses RELATES_TO edges, colored by type, no labels (hover only)
# mode = "cooccurrence" → uses CO_OCCURS_WITH edges, sized by count
# keyword = comma-separated string → filters to those entities + direct neighbors
# week = "2026-WXX" → filters co-occurrence mode to that week only
def build_graph_visual(keyword=None, week=None, mode="semantic"):
    ...
    # Parse comma-separated filter into list
    if keyword:
        kw_list = [k.strip().lower() for k in keyword.split(",") if k.strip()]
    else:
        kw_list = []
    ...
    with driver.session() as session:
        ...
        if mode == "semantic":
            if kw_list:
                # Step 1 — find seed entities matching keywords
                ...
                if not seeds:
                    # No matches — fall back to full graph
                    res = session.run("""
                    ...
                    # Step 2 — get direct neighbors of seeds
                    ...
                    # Step 3 — get node metadata for seeds + neighbors
            ...
            # Get semantic edges between top nodes
            rels = session.run("""
    ...
    # ── Build PyVis network ──
    # Edge labels hidden — only visible on hover via title property
    # Increased spring length for cleaner layout
    ...
    for r in relationships:
        if r["source"] in top and r["target"] in top:
            ...
            # No label on edge — relationship type shown only on hover
            net.add_edge(
                r["source"], r["target"],
...
# ── [SECTION: TAB 1 — GRAPH CONTROLS] ──
        ...
        with c_week:
            # Week selector only applies to CO-OCCURRENCE mode
            # Disabled in semantic mode since RELATES_TO has no week property
...
with tab2:
    ...
    if run_traj:
        ...
        if not weekly_data:
            ...
            # Semantic relationships between these two entities
            with driver.session() as session:
...
with tab4:
    ...
    if run_surge:
        ...
        if not surges:
            ...
            # Fallback — show top entities by connection strength
            with driver.session() as session:
                top = session.run("""
    ...
    if run_weekly_brief:
        ...
        with st.spinner("Analysing graph..."):
            with driver.session() as session:
                ...
                # Supporting headlines for each pair
                headlines = {}
                for p in top_pairs:
                    arts = session.run(
        ...
        # Generate and offer PDF download — uses more detailed prompt
