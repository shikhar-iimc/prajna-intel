...
def build_graph_visual(keyword=None, week=None, mode="semantic"):
    ...
    for name, data in top.items():
        ...
        size     = 32 if is_india else (22 if is_seed else min(8 + data["conn"] * 0.8, 22))
    ...
    for r in rows:
        for name, ntype in [(r["e1"], r.get("t1","ORG")), (r["e2"], r.get("t2","ORG"))]:
            if name not in nodes_seen:
                ...
                size  = 18 if ntype == "THREAT_ACTOR" else (15 if ntype == "SECTOR" else 10)
                net.add_node(name, label=name,
    ...
    for r in rows:
        for name, ntype in [(r["e1"], r.get("t1","SANCTIONED_ORG")), (r["e2"], r.get("t2","SANCTIONED_ORG"))]:
            if name not in nodes_seen:
                ...
                size  = 18 if ntype in ("SANCTIONED_PERSON","SANCTIONED_ORG") else 10
                net.add_node(name, label=name,
                             color={"background": color, "border": color,
                                    "highlight": {"background": "#E8D5A3", "border": "#FFFFFF"}},
                             size=size, title=f"{ntype}",
...
# ════════════════════════════════════════════════════════════════
    ...
    with rtab1:
        ...
        with col_graph:
            ...
            raksha_mode   = st.selectbox("Mode", ["semantic", "co-occurrence"], key="raksha_mode")
...
# ════════════════════════════════════════════════════════════════
    ...
    with atab2:
        ...
        with col_graph:
            ...
            fin_mode   = st.selectbox("Mode", ["semantic", "co-occurrence"], key="fin_mode")
