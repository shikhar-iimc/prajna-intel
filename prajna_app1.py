...
NEO4J_URI_2      = os.environ.get("NEO4J_URI_2")      or st.secrets.get("NEO4J_URI_2")
NEO4J_USERNAME_2 = os.environ.get("NEO4J_USERNAME_2") or st.secrets.get("NEO4J_USERNAME_2")
NEO4J_PASSWORD_2 = os.environ.get("NEO4J_PASSWORD_2") or st.secrets.get("NEO4J_PASSWORD_2")
# ── [END: CREDENTIALS] ──────────────────────────────────────────
...
if module == "PRAJNA — Strategic Intelligence":
    ...
    with tab1:
        ...
        col_mode, col_week_t1, col_run = st.columns([1, 2, 1])
        with col_mode:
            graph_mode = st.selectbox("GRAPH MODE", ["semantic", "co-occurrence"], key="graph_mode_t1")
        with col_week_t1:
            available_weeks_t1 = get_available_weeks()
            week_options_t1    = ["ALL WEEKS"] + available_weeks_t1
            selected_week_t1   = st.selectbox("WEEK FILTER (co-occurrence only)", week_options_t1, key="week_t1")
        with col_run:
